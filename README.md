...
### 자주 헷갈리는 포인트: 공격 종류인가, 시나리오인가?

둘 다 맞습니다.

- 개념적으로는 `Blackhole`, `Selective Forwarding`, `Sybil`, `Default`가 **악성 드론의 공격 동작 타입(behavior type)** 입니다.
- 실험 운영에서는 각 공격 동작 타입을 환경에 적용해 **평가 시나리오**로 실행합니다.

즉, 이 프로젝트에서는 "공격 종류를 시나리오로 돌린다"고 이해하면 가장 정확합니다.

### 그러면 결과도 4개가 나와야 하나?

네. 정상적으로 평가가 수행되면 시나리오별 결과가 4개(`Default`, `Blackhole`, `Selective Forwarding`, `Sybil`) 나와야 합니다.

- `eval_metrics*.csv`에서는 보통 `scenario` 컬럼으로 4개 시나리오가 분리 기록됩니다.
- 비교 요약 파일(`compare_*_summary.csv`)은 시나리오별 행 + 평균/집계 정보가 함께 있어, 한 개 결과처럼 보일 수 있습니다.

따라서 분석할 때는

1. 시나리오별 성능(4개)을 먼저 확인하고
2. 그 다음 평균/통합 지표(AUC, 평균 PDR/Delay 등)로 최종 비교하는 순서를 권장합니다.
